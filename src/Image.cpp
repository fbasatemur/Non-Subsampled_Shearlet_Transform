...
{
    int width = Buffer.cols(), height = Buffer.rows();
	// first make sure the parameters are valid
	if ((nullptr == Buffer.data()) || (width == 0) || (height == 0))
		return NULL;
	...
	for (int row = 0; row < height; row++)
		for (int column = 0; column < width; column++) {
            ...
            newbuf[newpos + 2] = newbuf[newpos + 1] = newbuf[newpos] = static_cast<BYTE>(Buffer(row, column));       //  red = green= blue = intensity
		}
